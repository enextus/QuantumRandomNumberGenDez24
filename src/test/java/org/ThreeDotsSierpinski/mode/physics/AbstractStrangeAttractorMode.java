...
import org.ThreeDotsSierpinski.mode.RngSampler;
...
abstract class AbstractStrangeAttractorMode implements VisualizationMode {
    ...
    private int nextRandomOrFrame(RNProvider provider) {
        RngSampler sampler = RngSampler.from(provider);
        var value = sampler.next();
        randomNumbersUsed += sampler.used();
        if (value.isPresent()) {
            return value.getAsInt();
        }
        return Math.floorMod(frame * 1103 + 97, RngSampler.UINT16_MAX + 1);
    }

    private double unit(int randomValue) {
        return RngSampler.toUnit(randomValue);
    }

    private double signedUnit(int randomValue) {
        return RngSampler.toSignedUnit(randomValue);
    }
